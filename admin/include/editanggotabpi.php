...
if(isset($_GET['data'])){
  ...
  //get data anggota BPI
  $sql_d = "SELECT `id_departemen`, `id_periode`, `nama_anggota`, `jabatan`, `status` FROM `anggota` 
  WHERE `id_anggota` = '$id_anggota'";
  $query_d = mysqli_query($koneksi,$sql_d);
  while($data_d = mysqli_fetch_row($query_d)){
      $id_departemen = $data_d[0];
      $id_periode    = $data_d[1];
      $nama_anggota  = $data_d[2];
      $jabatan       = $data_d[3];
      $status        = $data_d[4];
    }
    // get Proker 
    $sql_h ="SELECT `id_proker` FROM `anggota` 
    WHERE `id_anggota` = '$id_anggota'";
    $query_h      = mysqli_query($koneksi, $sql_h);
    $array_proker = array();
    While ($data_h = mysqli_fetch_row($query_h)){
      $id_proker      = $data_h[0];
      $array_proker[] = $id_proker;
    }
}
?>
...
<section class="content ">
  <div class="card card-info">
    ...
    <form class="form-horizontal" method="post" action="index.php?include=konfirmasi-edit-anggota-bpi" enctype="multipart/form-data">
      <div class="card-body">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-7">
            <select class="form-control" id="jabatan" name="jabatan" >
                ...
                <option value="Ketua">Ketua forum</option>
                <option value="Wakil">Wakil ketua forum</option>
